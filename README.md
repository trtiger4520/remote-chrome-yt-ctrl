...
Vite 遙控介面預設使用 `http://localhost:5173`，並將 `/hubs` 與 `/api` 代理至 `http://127.0.0.1:5080`。Server 本身在 `artifacts/remote-web` 存在時會使用該目錄提供靜態檔案
...
第一版是 LAN HTTP。Token 放在 URL fragment，不會隨首次頁面 request 傳送，但同網段的封包側錄仍可能取得後續 SignalR access token。請勿將 5080 直接暴露至公網，也不要將配對網址貼到公開頻道
...
Windows Firewall 請只針對私人網路 profile 建立 TCP 5080 入站規則，並由管理者依環境人工設定。程式不會自動修改 Firewall
...
- 確認 Server 正在 `127.0.0.1:5080` 監聽
