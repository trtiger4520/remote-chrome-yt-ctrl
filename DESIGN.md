...
- live 或 `canSeek=false`：停用進度與前後 seek，保留播放、音量、倍速、全螢幕、字幕與按讚
- YouTube 按讚：沿用目前登入 Chrome 分頁的按讚狀態，按鈕未載入或帳號無法操作時停用
- 音量控制：使用兩欄寬的緊湊水平滑桿，與快速控制維持同一控制區域
